    ...
    void shumanController()
    {
        if (Input.GetKeyDown(KeyCode.Mouse0) && interactor.isInteractable)
        {
            switch (ifInter.isHolding)
            {
                ...
                //Knife 
                case 2:
                    if (ifInter.whatwasHit() == "shoulder")
                    {
                        shoulder.SetActive(false);
                        meat.meatSelector(2);
                        break;
                    }
                    else if (ifInter.whatwasHit() == "chuck")
                    {
                        breast.SetActive(false);
                        meat.meatSelector(1);
                        break;
                    }
                    else if (ifInter.whatwasHit() == "abdomen")
                    {
                        medius.SetActive(false);
                        meat.meatSelector(4);
                        break;
                    }
                    else if (ifInter.whatwasHit() == "glute")
                    {
